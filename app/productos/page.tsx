    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-green-800 text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/dashboard" className="text-green-200 hover:text-white">← Dashboard</Link>
          <h1 className="text-xl font-bold">Productos</h1>
        </div>
        <button onClick={() => setShowForm(!showForm)} className="bg-green-600 hover:bg-green-500 px-4 py-2 rounded font-semibold">
          + Nuevo Producto
        </button>
      </nav>
          ...
          <div className="bg-white rounded-xl shadow overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {products.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-8 text-gray-400">No hay productos aún</td></tr>
                )}
              </tbody>
            </table>
          </div>
